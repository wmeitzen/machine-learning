...
def convert_to_tflite(keras_model, tflite_filename):
	converter = tf.lite.TFLiteConverter.from_keras_model(keras_model) # fails
	#converter = tf.lite.TFLiteConverter.from_saved_model(keras_model) # fails
...
def recreate_train_test_validate_images_subdirs():
	...
	for subdir in subdirs:
		...
		for labeldir in labeldirs:
			newdir = processed_dataset_home + subdir + labeldir
...
# - set both to False after images are processed and nn is trained
initialize_images = False # process images - set to True only for new or changed images
initialize_harness = False # process harness - set to True to re-train
...
tflite_filename = "final_model.tflite"
