...
class QLearning:
    def __init__(
        ...
        if os.path.exists(strSQLiteDB) == False:
            ...
            objDB.execute("create table modelinfo (trial integer primary key, ts timestamp default (datetime('now','localtime')) not null, average_run_length float, discount float, elapsed_learning_time_sec int, elapsed_learning_time_string varchar(64) default 'Processing ...', epsilon_decay float, episode_min float, learning_rate_decay float, lawn_size int, num_episodes int, perfect_run_length int);")
            objDB.execute("create unique index modelinfo_uq on modelinfo (discount, epsilon_decay, episode_min, learning_rate_decay, lawn_size, num_episodes);")
            objConn.commit()
            objConn.close()
    ...
    def launch_qlearning(self):
        ...
        objDB = objConn.cursor()
        strSQL = f"select trial from modelinfo where discount = {self.DISCOUNT} and epsilon_decay = {self.EPSILON_DECAY} and episode_min = {self.EPSILON_MIN} and learning_rate_decay = {self.LEARNING_RATE_DECAY} and lawn_size = {self.get_size()} and num_episodes = {self.NUM_EPSIODES};"
        ...
        strSQL = f"insert into modelinfo (discount, elapsed_learning_time_sec, elapsed_learning_time_string, epsilon_decay, episode_min, learning_rate_decay, lawn_size, num_episodes) values ({self.DISCOUNT}, {elapsed_time_sec}, '{elapsed_time_string}', {self.EPSILON_DECAY}, {self.EPSILON_MIN}, {self.LEARNING_RATE_DECAY}, {self.get_size()}, {self.NUM_EPSIODES});"
        strSQLiteDB = self.env.get_db_filename()
        objConn = sql.connect(strSQLiteDB)
        objDB = objConn.cursor()
        objDB.execute(strSQL)
