...
class BlobEnv:
    ...
    def is_obstacle(self, x, y):
        #return self.board[y, x] == self.keys["obstacle"]
        # -- horrible code, but debuggable
        r = (self.board[y, x] == self.keys["obstacle"]).all()
        if r == True:
            return True
        else:
            return False
    ...
    def step(
        ...
        if self.is_obstacle(x = new_mower_x, y = new_mower_y):
            new_mower_x = mower_x
            new_mower_y = mower_y

        #print(f"is_obstacle(new_mower_x={new_mower_x}, new_mower_y={new_mower_y})")
            #self.board = old_board
        #    new_mower_x = mower_x
        #    new_mower_y = mower_y
            #self.mower.set_x(new_mower_x)
            #self.mower.set_y(new_mower_y)

        self.board[mower_y, mower_x] = self.keys["mowed"]
        #if not(mower_x == new_mower_x and mower_y == new_mower_y):
        ...
        if action in [1, 2]:
            return self.board, self.change_direction_reward, self.remaining_squares_to_mow <= 0,
        ...
        if (old_board[new_mower_y, new_mower_x] == self.keys["unmowed"]).all():
            reward += self.unmowed_reward
            self.remaining_squares_to_mow -= 1
        # Reward (negative) if mowed an already mowed square of grass
        elif (old_board[new_mower_y, new_mower_x] == self.keys["mowed"]).all():
